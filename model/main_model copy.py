...
class VariVAE(DANIEL):
    def __init__(self, config):
        ...
        self.vae_j = VAE(self.embedding_output_dim, config.hidden_dim_vae, self.latent_dim).to(device)
        self.vae_m = VAE(self.embedding_output_dim, config.hidden_dim_vae, self.latent_dim).to(device)
        # Actor and Critic using VAE latent variable
        actor_input_dim = 2 * self.latent_dim + self.pair_input_dim
        self.actor = Actor(config.num_mlp_layers_actor, actor_input_dim, config.hidden_dim_actor, 1).to(device)

        critic_input_dim = 2 * self.embedding_output_dim
    ...
    def forward(self, fea_j, op_mask, candidate, fea_m, mch_mask, comp_idx, dynamic_pair_mask, fea_pairs, params=None):
        ...
        reconstructed_fea_j, mu_j, logvar_j = self.vae_j(fea_j)
        reconstructed_fea_m, mu_m, logvar_m = self.vae_m(fea_m)
        ...
        # mu_j_expanded = mu_j.unsqueeze(2).repeat(1, 1, M, 1).reshape(sz_b, M * J, self.latent_dim)
        mu_m_expanded = mu_m.unsqueeze(1).repeat(1, J, 1, 1).reshape(sz_b, M * J, self.latent_dim)
        fea_pairs = fea_pairs.reshape(sz_b, -1, self.pair_input_dim)
        candidate_feature = torch.cat((mu_j, mu_m_expanded, fea_pairs), dim=-1)
        ...
        return pi, v, mu_j, logvar_j, mu_m, logvar_m, fea_j, fea_m, reconstructed_fea_j, reconstructed_fea_m

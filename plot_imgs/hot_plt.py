...
def MAML_finetuning_test(args):
    ...
    model = test_model[0]
    if model[1] == "test_random":
        ppo = PPO_initialize(configs)
        torch.save(ppo.policy.state_dict(), f'./trained_network/{configs.model_source}/test_random.pth')
...
def MAML_finetuning_EC(model, dim=64):
    n_j_list = [5, 8, 11, 14, 17, 20, 23, 25]
    n_m_list = n_j_list
...
if __name__ == "__main__":
    # random
    MAML_finetuning_EC("maml+MAMLECpre_train1712286401", dim=512)

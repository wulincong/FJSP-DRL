...
def MAML_finetuning(model1, model2):
    ...
    args1 = ["--test_data", *test_data, 
        "--hidden_dim_actor", "512", "--hidden_dim_critic", "512",
         "--test_model", model1]
    

    args2 = ["--test_data", *test_data, 
         "--test_model", model2]
    
    # model1_res, _ = MAML_finetuning_test(args = args1)
    with open(f"./plot_imgs/hot_plt_data/model1_res{log_timestamp}.txt", "w") as file:
        print([], file = file)
    model2_res, _ = MAML_finetuning_test(args = args2)
    with open(f"./plot_imgs/hot_plt_data/model2_res{log_timestamp}.txt", "w") as file:
        print(model2_res, file = file)

if __name__ == "__main__":
    # random
    MAML_finetuning("maml+EXP19maml1709218650", "test_random")
